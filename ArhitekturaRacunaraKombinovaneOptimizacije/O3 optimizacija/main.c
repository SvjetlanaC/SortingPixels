...
void sortColumns(BMP_INFO info,unsigned char *buffer,int i2,int block,int k1)
{
    COLOR t;

    int p = getPadding(info);
...
void sort(BMP_INFO info, unsigned char *buffer,int block)
{
    ...
    #pragma omp parallel sections
    {
        ...
        #pragma omp section
        {
            ...
            for(int i1=0; i1 < pivot-1; i1+=block){
                 for(int k1=0; k1<info.width * 3; k1+=block){

                        for(int i2=i1; (i2<i1+block) && (i2<pivot-1); i2++)
                        {
                            sortColumns(info,buffer,i2,block,k1);
                        }
                    }
            }
        }
        ...
        #pragma omp section
        {
            for(int i1=pivot-1; i1 < info.height; i1+=block){
                 for(int k1=0; k1<info.width * 3; k1+=block){
...
int main(int argc, char *argv[])
{
	...
	else
	{
		if ((in = fopen(argv[1], "rb")) != NULL)
		{
			...
			else
			{
                ...
                printf("Vrijeme izvrsavanja sa kombinovanim optimizacijama: %ds%dms",total/1000,total%1000);
    ...
    {
        fprintf(info,"Vrijeme izvrsavanja sa kombinovanim optimizacijama: %ds%dms \n",total/1000,total%1000);
    }
    else
